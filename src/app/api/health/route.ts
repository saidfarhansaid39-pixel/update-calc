...
export async function GET(request: Request) {
  ...
  const refType = searchParams.get('references')

  if (refType) {
    const data: Record<string, any> = {}
    if (refType === 'all' || refType === 'bmi') data.bmi = BMI_CATEGORIES
    if (refType === 'all' || refType === 'calorie') data.calorie = CALORIE_REFERENCE
    if (refType === 'all' || refType === 'bodyfat') data.bodyFat = BODY_FAT_PERCENTAGES
    if (refType === 'all' || refType === 'bmr') data.bmr = BMR_FORMULAS
    if (refType === 'all' || refType === 'water') data.water = WATER_INTAKE
    if (refType === 'all' || refType === 'macros') data.macros = MACROS_REFERENCE
