...
const calcDef: CalcDef = {
  ...
  fields: [{ name:'fiO2', label:'FiO₂ (decimal)', type:'number', min:0.21, max:1, step:'0.01' }, { name:'paCO2', label:'PaCO₂ (mmHg)', type:'number', min:10, max:150, step:'1' }, { name:'paO2', label:'PaO₂ (mmHg)', type:'number', min:20, max:700, step:'1' }, { name:'age', label:'Age (years)', type:'number', min:1, max:110, step:'1' }],
  ...
  description: 'Alveolar-arterial oxygen gradient for evaluating hypoxemia etiology.',
  formula: 'A-a = (FiO₂×(760-47)-PaCO₂/0.8)-PaO₂. Expected = Age/4+4.',
  interpretation: 'Widened gradient indicates V/Q mismatch, shunt, or diffusion impairment. Normal on room air <10-20 mmHg.'
}
