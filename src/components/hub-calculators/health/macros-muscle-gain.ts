...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const w=parseFloat(v.weight)||70; const h=parseFloat(v.height)||175; const age=parseFloat(v.age)||30; const sex=v.sex||'male'; const act=parseFloat(v.activity)||1.55; const bmr=sex==='male'?10*w+6.25*h-5*age+5:10*w+6.25*h-5*age-161; const tdee=bmr*act; const surplus=tdee*1.15; const protein=Math.round(w*1.8); const fat=Math.round(w*0.9); const carbs=Math.round((surplus-protein*4-fat*9)/4); return { result:surplus, label:'Calories (Muscle Gain)', unit:'kcal/day', steps:[{ label:'BMR (Mifflin)', value:bmr.toFixed(0)+' kcal/day' },{ label:'TDEE', value:tdee.toFixed(0)+' kcal/day' },{ label:'Surplus (+15%)', value:surplus.toFixed(0)+' kcal/day' },{ label:'Protein', value:protein+' g (1.8g/kg)' },{ label:'Fat', value:fat+' g (0.9g/kg)' },{ label:'Carbs', value:carbs+' g' }] } },
  description: 'Calculate macronutrient targets for muscle gain based on Mifflin-St Jeor BMR and activity level.',
  formula: 'Protein: 1.8 g/kg bodyweight. Fat: 0.9 g/kg. Carbs: remaining calories. Caloric surplus: TDEE × 1.15.',
  interpretation: 'Aim for 0.25-0.5 kg weight gain per week. Adjust surplus if weight gain is too fast or too slow.'
}
