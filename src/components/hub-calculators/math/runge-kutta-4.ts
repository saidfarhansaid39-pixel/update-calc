...
const calcDef: CalcDef = {
    ...
    compute: (v) => {
      ...
      for (let i = 0; i < s; i++) { const k1 = h * (x + y); const k2 = h * ((x + h / 2) + (y + k1 / 2)); const k3 = h * ((x + h / 2) + (y + k2 / 2)); const k4 = h * ((x + h) + (y + k3)); y += (k1 + 2 * k2 + 2 * k3 + k4) / 6; x += h; st.push(step(`Step ${i + 1}`, `x=${x.toFixed(4)}, y=${y.toFixed(4)}`)) }
      return { result: y.toFixed(6), label: `y(${x.toFixed(4)})`, steps: st }
    },
    formula: 'RK4: k1 = hf(x,y), k2 = hf(x+h/2,y+k1/2), k3 = hf(x+h/2,y+k2/2), k4 = hf(x+h,y+k3).',
    description: 'Runge-Kutta 4th order method for ODEs.',
    interpretation: 'Fourth-order accurate ODE solver using four slope evaluations.'
}
