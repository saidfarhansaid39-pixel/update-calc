...
const calcDef: CalcDef = {
    ...
    compute: (v) => {
      ...
      for (let i = 2; i <= Math.min(fib, 20); i++) seq.push(seq[i - 1] + seq[i - 2])
      return { result, label: 'F(' + fib + ')', steps: [step('Sequence:', 'F(0)=0, F(1)=1'), step('Formula:', 'F(n) = F(n-1) + F(n-2)'), step('Result:', 'F(' + fib + ') = ' + result)], extras: [{ label: 'Sequence (to 20)', value: seq.join(', ') + (fib > 20 ? '...' : '') }] }
    },
