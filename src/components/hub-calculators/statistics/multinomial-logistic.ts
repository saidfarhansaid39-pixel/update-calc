...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const coeffs = parseList(v.coeffs); const x = n(v.x); if (coeffs.length < 1) return { result: 'Need ≥1 coefficient', label: '', unit: '', steps: [] ,
    ...
    ]}; const logits = coeffs.map(c => c * x); const expLogits = logits.map(l => Math.exp(l)); const sumExp = expLogits.reduce((a, b) => a + b, 0) + 1; const probs = expLogits.map(e => e / sumExp); const refProb = 1 / sumExp; return { result: `[${probs.map(p => p.toFixed(4)).join(', ')}, ref:${refProb.toFixed(4)}]`, label: 'Class Probabilities', unit: '', steps: [{ label: 'Categories', value: `${coeffs.length + 1} (incl. reference)` }, { label: 'Probabilities', value: probs.map((p, i) => `C${i + 1}:${(p * 100).toFixed(1)}%`).join(', ') + `, Ref:${(refProb * 100).toFixed(1)}%` }] } },
