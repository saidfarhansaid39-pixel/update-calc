...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const obs = parseList(v.observed); const exp = parseList(v.expected); if (obs.length !== exp.length || obs.length < 2) return { result: 'Need ≥2 categories', label: '', unit: '', steps: [] ,
    ...
    ]}; const chi2 = obs.reduce((acc, o, i) => acc + (exp[i] > 0 ? ((o - exp[i]) ** 2) / exp[i] : 0), 0); const df = obs.length - 1; return { result: chi2, label: 'χ² Goodness of Fit', unit: '', steps: [{ label: 'χ²', value: `${chi2.toFixed(4)}` }, { label: 'DF', value: `${df}` }] } },
