...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const vals = parseList(v.shares); if (vals.length < 2) return { result: 'Need ≥2 shares', label: '', unit: '', steps: [] ,
    ...
    ]}; const hhi = vals.reduce((acc, s) => acc + s * s, 0); const normHHI = (hhi - 10000 / vals.length) / (10000 - 10000 / vals.length); return { result: hhi, label: 'HHI', unit: '', steps: [{ label: 'Firms', value: `${vals.length}` }, { label: 'HHI', value: `${hhi.toFixed(2)}` }, { label: 'Normalized HHI', value: `${normHHI.toFixed(4)}` }, { label: 'Concentration', value: hhi < 1000 ? 'Unconcentrated' : hhi < 2500 ? 'Moderately concentrated' : 'Highly concentrated' }] } },
