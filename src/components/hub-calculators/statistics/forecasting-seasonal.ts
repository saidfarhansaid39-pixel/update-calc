...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const nums = parseList(v.values); const per = Math.round(n(v.period)); const h = Math.round(n(v.periods)); if (nums.length < 2 * per || per === 0) return { result: `Need ≥${Math.max(2, 2 * per)} values`, label: '', unit: '', steps: [] ,
    extras: [
