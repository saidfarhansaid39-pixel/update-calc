...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const nums = parseList(v.values); const alpha = n(v.alpha); if (nums.length < 2) return { result: 'Need ≥2 values', label: '', unit: '', steps: [] ,
    extras: [
