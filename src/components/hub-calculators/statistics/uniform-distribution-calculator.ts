...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const a = n(v.a); const b = n(v.b); const x = n(v.x); if (a >= b) return { result: 'Error', label: '', unit: '', steps: [] ,
    extras: [
    ...
    ]}; const pdf = (x >= a && x <= b) ? 1 / (b - a) : 0; const cdf = x < a ? 0 : x > b ? 1 : (x - a) / (b - a); const mean = (a + b) / 2; return { result: pdf, label: 'PDF', unit: '', steps: [{ label: 'PDF', value: `${pdf.toExponential(4)}` }, { label: 'CDF', value: `${cdf.toExponential(4)}` }, { label: 'Mean', value: `${mean.toFixed(4)}` }] } },
