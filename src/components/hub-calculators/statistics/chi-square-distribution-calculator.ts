...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const df = n(v.df); const x = n(v.x); const pdf = x > 0 ? Math.exp((df / 2 - 1) * Math.log(x) - x / 2 - (df / 2) * Math.log(2) - lgamma(df / 2)) : 0; return { result: pdf, label: 'PDF', unit: '', steps: [{ label: 'χ² value', value: `${x.toFixed(4)}` }, { label: 'Degrees of freedom', value: `${df}` }, { label: 'PDF', value: `${pdf.toExponential(4)}` }] ,
    extras: [
