...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const d1 = n(v.df1); const d2 = n(v.df2); const x = n(v.x); const pdf = x > 0 ? Math.exp((d1 / 2) * Math.log(d1) + (d2 / 2) * Math.log(d2) + lgamma((d1 + d2) / 2) - lgamma(d1 / 2) - lgamma(d2 / 2) + (d1 / 2 - 1) * Math.log(x) - ((d1 + d2) / 2) * Math.log(d1 * x + d2)) : 0; return { result: pdf, label: 'PDF', unit: '', steps: [{ label: 'F-value', value: `${x.toFixed(4)}` }, { label: 'DF (num, den)', value: `${d1}, ${d2}` }, { label: 'PDF', value: `${pdf.toExponential(4)}` }] ,
    extras: [
    ...
    ]}; function lgamma(z: number): number { return (z - 0.5) * Math.log(z + 4.5) - (z + 4.5) + 0.9189385332 } },
