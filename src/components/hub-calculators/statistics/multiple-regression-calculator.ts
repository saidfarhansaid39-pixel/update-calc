...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const x1 = parseList(v.x1); const x2 = parseList(v.x2); const y = parseList(v.y); if (x1.length !== x2.length || x1.length !== y.length || x1.length < 3) return { result: 'Need ≥3 rows', label: '', unit: '', steps: [] ,
    extras: [
