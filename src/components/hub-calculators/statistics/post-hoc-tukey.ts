...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const means = parseList(v.means); const nPer = Math.round(n(v.n)); const mse = n(v.mse); if (means.length < 2) return { result: 'Need ≥2 groups', label: '', unit: '', steps: [] ,
    ...
    ]}; const se = Math.sqrt(mse / nPer); const qs = []; for (let i = 0; i < means.length; i++) { for (let j = i + 1; j < means.length; j++) { const diff = Math.abs(means[i] - means[j]); const q = se > 0 ? diff / se : 0; qs.push({ pair: `${i + 1}-${j + 1}`, diff: diff.toFixed(4), q: q.toFixed(4) }) } }; return { result: qs.map(q => `${q.pair}:${q.q}`).join('; '), label: 'Tukey HSD Q Stats', unit: '', steps: [{ label: 'Groups', value: `${means.length}` }, { label: 'SE', value: `${se.toFixed(4)}` }, { label: 'Comparisons', value: qs.map(q => `${q.pair} q=${q.q}`).join(', ') }] } },
