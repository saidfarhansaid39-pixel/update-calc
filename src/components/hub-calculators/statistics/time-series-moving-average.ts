...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const nums = parseList(v.values); const w = Math.round(n(v.window)); if (nums.length < w) return { result: `Need ≥${w} values`, label: '', unit: '', steps: [] ,
    ...
    ]}; const ma: number[] = []; for (let i = w - 1; i < nums.length; i++) { ma.push(nums.slice(i - w + 1, i + 1).reduce((a, b) => a + b, 0) / w) }; const raw = nums.slice(-5).map(x => x.toFixed(2)).join(', '); const smoothed = ma.slice(-5).map(x => x.toFixed(2)).join(', '); return { result: ma[ma.length - 1], label: 'Last Moving Average', unit: '', steps: [{ label: 'Window', value: `${w}` }, { label: 'Last raw', value: raw }, { label: 'Last smoothed', value: smoothed }, { label: 'MA values', value: ma.map(x => x.toFixed(2)).join(', ') }] } },
