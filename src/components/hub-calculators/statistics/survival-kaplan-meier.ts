...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const t = parseList(v.times); const e = parseList(v.events); if (t.length !== e.length || t.length < 2) return { result: 'Need ≥2 obs', label: '', unit: '', steps: [] ,
    ...
    ]}; const pairs = t.map((ti, i) => ({ t: ti, e: e[i] })).sort((a, b) => a.t - b.t); let surv = 1; const survProbs = pairs.map((p, i) => { const nRisk = pairs.length - i; if (nRisk > 0 && p.e === 1) { surv *= (nRisk - 1) / nRisk }; return { t: p.t, surv } }); const medianIdx = survProbs.findIndex(sp => sp.surv <= 0.5); const medianTime = medianIdx >= 0 ? survProbs[medianIdx].t : t.reduce((a, b) => Math.max(a, b), 0); return { result: medianTime, label: 'Median Survival Time', unit: '', steps: [{ label: 'Observations', value: `${t.length}` }, { label: 'Events', value: `${e.filter(x => x === 1).length}` }, { label: 'Median survival', value: `${medianTime.toFixed(4)}` }, { label: 'Last survival prob', value: `${survProbs[survProbs.length - 1].surv.toFixed(4)}` }] } },
