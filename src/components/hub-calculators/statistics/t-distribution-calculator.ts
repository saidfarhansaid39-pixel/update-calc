...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const df = n(v.df); const t = n(v.t); const pdf = Math.exp(lgamma((df + 1) / 2) - lgamma(df / 2)) / (Math.sqrt(df * Math.PI) * Math.pow(1 + t * t / df, (df + 1) / 2)); return { result: pdf, label: 'PDF', unit: '', steps: [{ label: 't-value', value: `${t.toFixed(4)}` }, { label: 'Degrees of freedom', value: `${df}` }, { label: 'PDF', value: `${pdf.toExponential(4)}` }] ,
    extras: [
