...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const x = parseList(v.xValues); const y = parseList(v.yValues); if (x.length !== y.length || x.length < 2) return { result: 'Need ≥2 pairs', label: '', unit: '', steps: [] ,
    ...
    ]}; const mx = x.reduce((a, b) => a + b, 0) / x.length; const my = y.reduce((a, b) => a + b, 0) / y.length; const cov = x.reduce((acc, xi, i) => acc + (xi - mx) * (y[i] - my), 0) / (x.length - 1); return { result: cov, label: 'Covariance (sample)', unit: '', steps: [{ label: 'Mean X', value: `${mx.toFixed(4)}` }, { label: 'Mean Y', value: `${my.toFixed(4)}` }, { label: 'Covariance', value: `${cov.toFixed(4)}` }] } },
