...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const nums = parseList(v.values); const w = Math.round(n(v.window)); if (nums.length < w) return { result: `Need ≥${w} values`, label: '', unit: '', steps: [] ,
    ...
    ]}; const ma: number[] = []; for (let i = w - 1; i < nums.length; i++) { ma.push(nums.slice(i - w + 1, i + 1).reduce((a, b) => a + b, 0) / w) }; return { result: ma[ma.length - 1], label: 'Last SMA', unit: '', steps: [{ label: 'Window', value: `${w}` }, { label: 'SMA values', value: ma.map(m => m.toFixed(2)).join(', ') }, { label: 'Latest', value: `${ma[ma.length - 1].toFixed(4)}` }] } },
