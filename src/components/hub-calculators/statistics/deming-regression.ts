...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const x = parseList(v.xValues); const y = parseList(v.yValues); const lambda = n(v.ratio); if (x.length !== y.length || x.length < 3) return { result: 'Need ≥3 pairs', label: '', unit: '', steps: [] ,
    extras: [
