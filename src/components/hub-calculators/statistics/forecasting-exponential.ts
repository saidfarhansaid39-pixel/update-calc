...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const nums = parseList(v.values).filter(x => x > 0); const h = Math.round(n(v.periods)); if (nums.length < 2 || h === 0) return { result: 'Need ≥2 values and ≥1 period', label: '', unit: '', steps: [] ,
    extras: [
