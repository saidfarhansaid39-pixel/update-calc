...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const gs = [parseList(v.group1), parseList(v.group2), parseList(v.group3)].filter(g => g.length > 0); if (gs.length < 2) return { result: 'Need ≥2 groups', label: '', unit: '', steps: [] ,
    ...
    ]}; const all = gs.flat().map((x, i) => ({ val: x, idx: i })); const sorted = [...all].sort((a, b) => a.val - b.val); sorted.forEach((item, i) => item.idx = i + 1); let rankIdx = 0; for (let i = 0; i < sorted.length; i++) { let j = i; while (j + 1 < sorted.length && sorted[j + 1].val === sorted[i].val) j++; const avgRank = (i + 1 + j + 1) / 2; for (let k = i; k <= j; k++) sorted[k].idx = avgRank; i = j } const N = sorted.length; const allRanks = sorted.map(s => s.idx); const sumRank = allRanks.reduce((a, b) => a + b, 0); const H = gs.reduce((acc, g, gi) => { const ri = gs.slice(0, gi).reduce((s, prev) => s + prev.length, 0); const ranks = g.map((_, j) => allRanks[ri + j]); const rSum = ranks.reduce((a, b) => a + b, 0); return acc + rSum * rSum / g.length }, 0); const Hstat = (12 / (N * (N + 1))) * H - 3 * (N + 1); return { result: Hstat, label: 'H-Statistic', unit: '', steps: [{ label: 'Groups', value: `${gs.length}` }, { label: 'Total N', value: `${N}` }, { label: 'H', value: `${Hstat.toFixed(4)}` }] } },
