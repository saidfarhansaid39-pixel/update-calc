...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const z = (n(v.sampleMean) - n(v.popMean)) / (n(v.popStd) / Math.sqrt(n(v.n))); return { result: z, label: 'Z-Statistic', unit: '', steps: [{ label: 'Formula', value: 'z = (x̄ - μ₀) / (σ/√n)' }, { label: 'Standard error', value: `${(n(v.popStd) / Math.sqrt(n(v.n))).toFixed(4)}` }, { label: 'z', value: `${z.toFixed(4)}` }] } },
  description: 'A z-test compares a sample mean to a known population mean when population standard deviation is known.',
  formula: 'z = (x̄ - μ₀) / (σ / √n)',
  interpretation: '|z| > 1.96 is significant at α = 0.05 (two-tailed). |z| > 2.576 is significant at α = 0.01.'
}
