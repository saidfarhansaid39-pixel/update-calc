...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const rows = parseList(v.rowVar); const cols = parseList(v.colVar); if (rows.length !== cols.length || rows.length < 2) return { result: 'Need ≥2 pairs', label: '', unit: '', steps: [] ,
    extras: [
