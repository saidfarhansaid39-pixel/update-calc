...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const c1 = parseList(v.condition1); const c2 = parseList(v.condition2); const c3 = parseList(v.condition3); if (c1.length !== c2.length || c1.length !== c3.length || c1.length < 2) return { result: 'Need ≥2 subjects', label: '', unit: '', steps: [] ,
    ...
    ]}; const n = c1.length; const k = 3; const colSums = [c1.reduce((a, b) => a + b, 0), c2.reduce((a, b) => a + b, 0), c3.reduce((a, b) => a + b, 0)]; const rowSums = c1.map((_, i) => c1[i] + c2[i] + c3[i]); const grandSum = colSums.reduce((a, b) => a + b, 0); const num = (k - 1) * (k * colSums.reduce((acc, g) => acc + g * g, 0) - grandSum * grandSum); const den = k * grandSum - rowSums.reduce((acc, r) => acc + r * r, 0); const Q = den > 0 ? num / den : 0; return { result: Q, label: 'Cochran Q', unit: '', steps: [{ label: 'Col sums', value: colSums.join(', ') }, { label: 'Q', value: `${Q.toFixed(4)}` }] } },
