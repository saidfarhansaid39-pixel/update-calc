...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const t = (n(v.sampleMean) - n(v.hypMean)) / (n(v.sampleStd) / Math.sqrt(n(v.n))); const df = n(v.n) - 1; return { result: t, label: 't-Statistic', unit: '', steps: [{ label: 'Formula', value: 't = (x̄ - μ₀) / (s/√n)' }, { label: 'Standard error', value: `${(n(v.sampleStd) / Math.sqrt(n(v.n))).toFixed(4)}` }, { label: 't', value: `${t.toFixed(4)}` }, { label: 'DF', value: `${df}` }] } },
  ...
  interpretation: 'The t-statistic follows a t-distribution with n-1 degrees of freedom. Used when σ is unknown (most practical cases).'
}
