...
const calcDef: CalcDef = {
  ...
  compute: (v) => { const k = n(v.shape); const theta = n(v.rate); const x = n(v.x); const lnPdf = (k - 1) * Math.log(x) - x / theta - k * Math.log(theta) - lgamma(k); const pdf = Math.exp(lnPdf); const mean = k * theta; return { result: pdf, label: 'PDF', unit: '', steps: [{ label: 'PDF', value: `${pdf.toExponential(4)}` }, { label: 'Mean', value: `${mean.toFixed(4)}` }] ,
    extras: [
