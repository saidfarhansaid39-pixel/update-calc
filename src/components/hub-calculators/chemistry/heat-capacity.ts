...
const calcDef: CalcDef = {
  ...
  fields: [
    { name: 'mass', label: 'Mass', type: 'number', unit: 'g', min: 0.1, step: '0.1' },
  ...
  compute: (v) => {
    ...
    return {
      ...
      steps: [
        ...
        { label: 'Specific heat (c)', value: `${v.c} J/(g·°C)` },
        { label: 'ΔT', value: `${v.dT >= 0 ? '+' : ''}${v.dT} °C` },
        { label: 'q = mcΔT', value: `${q.toFixed(2)} J (${(q / 1000).toFixed(4)} kJ)` },
      ]
}
  },
  description: 'Heat capacity calculation using q = mcΔT. Specific heat capacity (c) is the energy needed to raise 1 g of substance by 1°C.',
  formula: 'q = mcΔT',
  interpretation: 'Water has high specific heat (4.184 J/g·°C), making it an excellent coolant. Substances with low specific heat heat up and cool down quickly.'
}
